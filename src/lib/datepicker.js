    ...
    _render() {
        this.shadowRoot.innerHTML = `
            <style>
                ...
                .custom-date input.year { width: 2.2em; }
                .custom-date input.month, .custom-date input.day { width: 1.3em; }
                .custom-date span { opacity: 0.6; margin: 0 -1px; line-height: 1; }
                .custom-date button.calendar-btn { position: relative; border: none; background: none; cursor: pointer; line-height: 1; padding: 0; width: 1.3em; min-width: 1.3em; height: 100%; flex: 0 0 1.3em; font-size: 0; display: flex; align-items: center; justify-content: center; color: #6b7280; }
                .custom-date button.calendar-btn::before { content: ""; display: block; width: .85em; height: .75em; border: 1.5px solid currentColor; border-radius: 2px; background: linear-gradient(to bottom, currentColor 0 22%, transparent 22% 100%); box-sizing: border-box; }
                .custom-date button.calendar-btn::after { content: ""; position: absolute; width: .45em; height: .22em; margin-top: -.62em; border-left: 1px solid currentColor; border-right: 1px solid currentColor; }
